...
func handlerGoroutine(consumer *Consumer, msgs <-chan amqp.Delivery, consumeOptions ConsumerOptions, handler Handler) {
	for msg := range msgs {
		...
		case Ack:
			err := msg.Ack(false)
			if err != nil {
				consumer.options.Logger.Error("ERR_CONSUMER_ACK", fmt.Errorf("can't ack message: %v", err))
			}
		case NackDiscard:
			err := msg.Nack(false, false)
			if err != nil {
				consumer.options.Logger.Error("ERR_CONSUMER_NACK", fmt.Errorf("can't nack message: %v", err))
			}
		case NackRequeue:
			err := msg.Nack(false, true)
			if err != nil {
				consumer.options.Logger.Error("ERR_CONSUMER_NACK-REQUEUE", fmt.Errorf("can't nack requeue message: %v", err))
			}
		}
	}
