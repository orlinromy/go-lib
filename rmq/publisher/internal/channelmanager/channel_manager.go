...
import (
	...
	"github.com/kelchy/go-lib/rmq/publisher/internal/connectionmanager"
	"github.com/kelchy/go-lib/rmq/publisher/internal/dispatcher"
	"github.com/kelchy/go-lib/rmq/publisher/internal/logger"
...
func (chanManager *ChannelManager) startNotifyCancelOrClosed() {
	...
	case err := <-notifyCloseChan:
		if err != nil {
			chanManager.logger.Error("ERR_RMQ-CHAN-MANAGER_START-NOTIFY-CANCEL-OR-CLOSED", fmt.Errorf("attempting to reconnect to amqp server after close with error: %v", err))
			chanManager.reconnectLoop()
			chanManager.logger.Out("OK_RMQ-CHAN-MANAGER_START-NOTIFY-CANCEL-OR-CLOSED", "successfully reconnected to amqp server after close")
			chanManager.dispatcher.Dispatch(err)
		}
		if err == nil {
			chanManager.logger.Out("OK_RMQ-CHAN-MANAGER_START-NOTIFY-CANCEL-OR-CLOSED", "amqp channel closed gracefully")
		}
	case err := <-notifyCancelChan:
		chanManager.logger.Error("ERR_RMQ-CHAN-MANAGER_START-NOTIFY-CANCEL-OR-CLOSED", fmt.Errorf("attempting to reconnect to amqp server after cancel with error: %v", err))
		chanManager.reconnectLoop()
		chanManager.logger.Out("OK_RMQ-CHAN-MANAGER_START-NOTIFY-CANCEL-OR-CLOSED", "successfully reconnected to amqp server after cancel")
...
func (chanManager *ChannelManager) reconnectLoop() {
	for {
		...
		if err != nil {
			chanManager.logger.Error("ERR_RMQ-CHAN-MANAGER_RECONNECT-LOOP", fmt.Errorf("error reconnecting to amqp server: %v", err))
		} else {
...
func (chanManager *ChannelManager) reconnect() error {
	...
	if err = chanManager.channel.Close(); err != nil {
		chanManager.logger.Error("WARN_RMQ-CHAN-MANAGER_RECONNECT", fmt.Errorf("error closing channel while reconnecting: %v", err))
	}
...
func (chanManager *ChannelManager) Close() error {
	chanManager.logger.Out("OK_RMQ-CHAN-MANAGER_CLOSE", "closing channel manager...")
	chanManager.channelMux.Lock()
	defer chanManager.channelMux.Unlock()
