...
func (connManager *ConnectionManager) Close() error {
	connManager.logger.Out("OK_RMQ-CONN-MANAGER_CLOSE", "closing connection manager...")
...
func (connManager *ConnectionManager) startNotifyClose() {
	...
	err := <-notifyCloseChan
	if err != nil {
		connManager.logger.Error("ERR_RMQ-CONN-MANAGER_START-NOTIFY_CLOSE", fmt.Errorf("attempting to reconnect to amqp server after connection close with error: %v", err.Error()))
		connManager.reconnectLoop()
		connManager.logger.Out("OK_RMQ-CONN-MANAGER_START-NOTIFY-CLOSE", "successfully reconnected to amqp server")
		connManager.dispatcher.Dispatch(err)
	}
	if err == nil {
		connManager.logger.Out("OK_RMQ-CONN-MANAGER", "amqp connection closed gracefully")
	}
}
...
func (connManager *ConnectionManager) reconnectLoop() {
	for {
		...
		if err != nil {
			connManager.logger.Error("ERR_RMQ_RECONNECT-LOOP", fmt.Errorf("error reconnecting to amqp server: %v", err))
		} else {
...
func (connManager *ConnectionManager) reconnect() error {
	...
	if err = connManager.connection.Close(); err != nil {
		connManager.logger.Error("WARN_RMQ-CONN-MANAGER_RECONNECT", fmt.Errorf("error closing connection while reconnecting: %v", err))
	}
