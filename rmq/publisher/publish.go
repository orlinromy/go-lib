...
import (
	...
	"github.com/kelchy/go-lib/rmq/publisher/internal/channelmanager"
...
func NewPublisher(conn *Conn, optionFuncs ...func(*PublisherOptions)) (*Publisher, error) {
	...
	go func() {
		for err := range publisher.reconnectErrCh {
			publisher.options.Logger.Out("OK_PUBLISH_RECONNECT", fmt.Sprintf("successful publisher recovery from: %v", err))
			err := publisher.startup()
			if err != nil {
				publisher.options.Logger.Error("ERR_PUBLISH_RECONNECT", fmt.Errorf("publisher closing, unable to recover from error on startup for publisher after cancel or close: %v", err))
				return
			}
...
func (publisher *Publisher) Close() {
	...
	if err != nil {
		publisher.options.Logger.Error("WARN_RMQ-PUBLISH_CLOSE", fmt.Errorf("error while closing the channel: %v", err))
	}
	publisher.options.Logger.Out("RMQ-PUBLISH_CLOSE", "closing publisher")
	go func() {
